...
def _login_and_expand_mymenu(driver, id, password):
    ...
    # マイメニュー展開
    driver.find_element(
        By.XPATH, '/html/body/header/div/div[4]/div/nav/ul/li[6]/button').click()
...
def download_withdrawal_history(id, password, download_dir):
    ...
    driver = _setup_driver(download_dir)

    try:
        _login_and_expand_mymenu(driver, id, password)
    except Exception as e:
        print(e)
        driver.get_screenshot_as_file(
            pathlib.Path('./debug_info', 'error.png'))
...
def download_dividened_history(id, password, download_dir):
    ...
    driver.find_element(
        By.XPATH, '/html/body/div[2]/div/div/div/div/table/tbody/tr/td/form/div[3]/table/tbody/tr[7]/td/span/input').click()
    driver.find_element(By.XPATH, '//img[@alt="CSVで保存"]').click()
